...
原生事件和合成事件的区别：原生事件指的是js中原始的事件 onclick。合成事件是指react中的事件 onClick。

合成事件处理了浏览器兼容性。

**原生事件**：不同浏览器对事件的支持和行为可能存在差异（如 IE 中的 `attachEvent`）。

**合成事件**：框架内部已处理好浏览器兼容性问题，开发者无需关心不同浏览器的差异。
...
1、event.stopPropagation(); 阻止事件的进一步传播，从而防止事件冒泡到其他元素

2、event.preventDefault(); 用于阻止事件的默认行为，但不影响事件的传播。

3、**return false**同时阻止了事件的默认行为和事件冒泡。在事件处理函数中返回`false`即可实现这一效果。
...
2、this.forceUpdate() 渲染组件，避免频繁使用 forceupdate，会强制渲染组件，不符合 React 设计思路。

此操作会跳过当前组件的 shouldComponentUpdate()。但其子组件会触发正常的生命周期方法，包括 shouldComponentUpdate() 方法。

#### 新版本 hook 函数组件

useEffect 中在组件更新时执行，当 props 或者 state 变化后，判断是否执行对应的函数
...
2、原生 js 层面：包括算法，缓存，从时间和空间分析

3、react 组件层面：减少不必要的渲染，有三种方法。 Should component update pure component reactmemo
...
mapDispatchToProps 将派发 action 的方法映射到子组件的 props 上。

<https://zhuanlan.zhihu.com/p/26648239>

[https://github.com/Michael18811380328/react-and-redux](https://github.com/Michael18811380328/react-and-redux "https://github.com/Michael18811380328/react-and-redux")
...
在初始渲染期间，返回的状态与作为第一个参数传递的值匹配。该`setState`函数用于更新状态。它采用新的状态值作为参数，并对组件的重新渲染进行排队。该`setState`函数还可以接受回调函数作为参数，该函数将之前的状态值作为参数。

实际上就是类组件 this.setState 状态更新的语法糖

参考官网：[https://react.dev/reference/react/useState](https://react.dev/reference/react/useState "https://react.dev/reference/react/useState")

内部实现：[https://juejin.cn/post/7076456859611168776](https://juejin.cn/post/7076456859611168776 "https://juejin.cn/post/7076456859611168776")
...
它通过引入**单向数据流**，和**状态的集中存储**，来帮助管理应用程序的状态。
...
Linters 是用于检查源代码是否存在潜在错误、错误、风格不一致和可维护性问题的工具。

Linters 工作原理：是扫描源代码，并将其与一组预定义的规则或指南进行比较。这些规则可以包括语法和格式约定、最佳实践、潜在错误。当 linter 发现违反规则时，它会生成警告或错误，突出显示需要注意的特定行或多行代码。
...
在 React 16 和之前的版本中，需要在开头引入 react 然后才能使用 jsx 语法。

在 React 17 和之后的版本中，不需要引入 react 也可以使用 jsx 语法（需要使用新 JSX 转换的打包工具，像 Babel 7.9.0 及以上版本或者 TypeScript 2.1 及以上版本）。

```javascript
// 新版本 JSX 转换（不需要引入 React）
function App() {
...
原因：React 17+ 不再强制引入 `React` 的核心原因是 JSX 转换机制的升级。新转换直接使用 `react/jsx-runtime` 中的辅助函数，避免了对 `React.createElement` 的依赖，从而消除了对 `React` 导入的强制要求。这一变化不仅简化了代码，还优化了构建结果。

如果有的项目 react 版本超过 17 但是不引入 react 会出错，那么检查一下配置的 babel 版本是否正确。
