...
性能上都一样，一个按照深度节点遍历，一个优先按照层遍历。

BFS 会用到一个额外的数组，存放队列，所以占用内存稍微多一点。

   
## 0006 DFS和BFS分别实现深拷贝一个对象


不考虑环形引用，那么就循环对象的键值对，然后分别拷贝即可（函数递归调用自己）
...
TCP 中的滑动窗口（Sliding Window）算法的复杂度，主要取决于窗口的大小，和数据包的传输频率。

在理想情况下，滑动窗口算法的时间复杂度为 O(1)，因为发送方和接收方只需要维护一个窗口的状态，发送和接收数据包时只需要更新窗口的边界。

但是，在实际实现中，滑动窗口算法的复杂度可能会受到以下因素的影响：

* 窗口大小的调整：当窗口大小需要调整时，发送方和接收方需要更新窗口的状态，这可能会导致 O(log n) 的复杂度，其中 n 是窗口大小。

* 数据包的传输频率：当数据包的传输频率很高时，发送方和接收方需要频繁地更新窗口的状态，这可能会导致 O(n) 的复杂度，其中 n 是数据包的数量。

* 丢包和重传：当数据包丢失或需要重传时，发送方和接收方需要更新窗口的状态并重新发送数据包，这可能会导致 O(n) 的复杂度，其中 n 是数据包的数量。

综上所述，TCP 中的滑动窗口算法的复杂度主要取决于窗口大小和数据包传输频率，在理想情况下为 O(1)，但在实际实现中可能会受到窗口大小调整、数据包传输频率和丢包重传等因素的影响，导致复杂度为 O(log n) 或 O(n)。
...
## 0745 Dijkstra 最短路径算法


狄克斯特拉算法：是从一个顶点到其余各顶点的最短路径算法，解决的是有权图中最短路径问题。

迪杰斯特拉算法主要特点是从起始点开始，采用贪心算法的策略，每次遍历到始点距离最近，且未访问过的顶点的邻接节点，直到扩展到终点为止。

假设我们有一个无向图，边的权重为正值，并且我们将使用一个邻接矩阵来表示图。
