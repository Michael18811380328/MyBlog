...
### rag 是什么
RAG 是 Retrieval-Augmented Generation 的缩写，“检索增强生成”。

它是一种让大模型在回答问题前，先去私有资料里“查一查”，再基于查到的内容来生成答案的方法。

​

解决什么问题？

1、大模型本身不一定知道你的私有知识库、最新文档或公司内部资料。

2、直接让模型“凭记忆回答”容易出现幻觉，也就是编造看起来很对但其实不准确的内容。

3、RAG 通过把“检索”接到“生成”前面，能让答案更可靠、更可追溯。
...
3. 点右上角**设置 → Skills**，能看到 `skills` 已被识别
...
如果你最近用过 Cursor、Claude Code 或其他 AI 编程工具，可能会发现一种有趣的变化：以前，我们习惯告诉 AI“怎么写”，比如“帮我实现一个登录接口”或“生成一个 CRUD 页面”；而现在，越来越多开发者开始直接描述“想要什么”，例如“做一个像 Notion 一样简洁的知识库”，“给用户一种流畅、轻量的使用体验”。这种从“描述实现”到“描述目标”的变化，正是 Vibe Coding 的核心。随着大模型推理能力和 Agent 能力不断提升，AI不再只是一个代码补全工具，而开始承担更多开发任务。开发者与 AI 的协作方式，也正在悄然改变。

## **Vibe Coding 到底是什么？​**

第一次听到 Vibe Coding，很多人会误以为它是一种新的编程语言，或者某种“凭感觉写代码”的开发方式。实际上，它更像是一种新的协作模式。传统开发中，程序员需要告诉计算机每一步应该怎么做；而在 Vibe Coding 中，开发者更关注的是最终效果，由 AI 自主完成中间的大部分实现过程。

例如，我们希望开发一个博客系统，传统方式可能会这样描述：
...
而 Vibe Coding 更可能只有一句话：

```
帮我做一个类似 Medium 的博客，重点突出阅读体验，界面尽量简洁。
```

剩下的页面布局、组件选择，甚至部分技术栈，AI 都会主动给出方案。换句话说，程序员开始从“代码实现者”转向“目标定义者”，AI 则承担越来越多的具体执行工作。

## **从 Prompt 到 Harness：Vibe Coding 为什么有效？​**

很多人认为，Vibe Coding 的核心在于 Prompt——只要提示词写得足够详细，AI 就能写出高质量代码。但在实际开发中，越来越多团队发现，Prompt 只是起点，真正决定 AI 能否完成任务的，是后续的验证与反馈机制，也就是常说的 Harness。可以把 Harness 理解成一个“自动验收系统”。AI 生成代码后，并不会直接结束，而是立即进入测试、检查、修复的循环。例如，一个典型流程可能是：
...
如果测试没有通过，AI 就继续修改，直到满足预期结果。举个简单的例子，假设我们让 AI 实现一个判断回文字符串的函数：
...
如果只是生成代码，到这里任务就结束了。但在 Harness 环境下，系统还会自动运行测试：

```
assert is_palindrome("level")
assert not is_palindrome("hello")assert is_palindrome("level")
```

一旦测试失败，AI 会根据错误信息继续修正代码，而不是等待开发者手动复制报错、重新提问。因此，现代 AI Coding 的工作方式，已经逐渐从“Prompt → 生成代码”，演变为“Prompt→ 生成代码 → 自动验证 → 自动修复”的闭环。Prompt 决定 AI 要做什么，而 Harness 则决定 AI 是否真的做对了。这也是 Vibe Coding 能够越来越接近真实软件开发的重要原因。

## **“无为而治”只是巧合吗？​**

聊到 Vibe Coding，很多人都会开玩笑：“这不就是《道德经》里的‘无为而治’吗？”这种说法虽然有些戏谑，但背后确实反映了一种开发思维的变化。过去的软件开发，更强调“控制过程”。程序员需要明确告诉计算机每一步应该怎么做，流程越细，结果越可控。而在 Vibe Coding 中，我们更关心的是“最终目标”。例如，与其告诉 AI：

```
先创建数据库，再写 Controller，然后实现 Service……
```

不如直接告诉它：

```
帮我做一个支持邮箱登录、界面简洁、适合中小团队使用的后台管理系统。
```

剩下的大部分实现细节，则交给 AI 去完成。这和《道德经》中强调的“少一些人为干预，多一些顺势而为”有一点相似，因此不少开发者会把 Vibe Coding 戏称为“编程禅宗”。当然，这只是一个有趣的类比，并不是 AI 真的理解了道家思想。真正发生变化的，是人与 AI 的协作方式：程序员开始减少对实现细节的控制，把更多精力放在定义目标和判断结果上。

## **Vibe Coding 什么时候会失效？​**

看到这里，可能有人会问：既然 Vibe Coding 这么高效，是不是以后所有项目都可以交给 AI？答案并没有这么乐观。对于个人工具、小型网站、MVP 或原型开发，Vibe Coding 往往能取得不错的效果。因为项目规模较小，AI 能够在有限的上下文中理解整个系统。但当项目越来越复杂时，问题就开始出现了。举个例子，如果一个商城项目不断增加优惠券、会员积分、库存管理、物流跟踪等功能，AI 在修改新功能时，很可能会影响之前已经写好的代码。开发者经常会遇到这样的情况：“修好了 A，结果 B 又坏了。”这并不是因为 AI 不会写代码，而是它需要同时关注的信息越来越多。随着项目规模扩大，模型的注意力会逐渐分散，很难始终保持对整个系统架构的完整理解。

因此，大型项目真正依赖的，仍然是软件工程中的架构设计、技术文档、测试体系和持续集成流程。这些工程化手段，本质上是在帮助 AI 和开发者共同维护系统的一致性，而不是依赖一次“完美的 Prompt”解决所有问题。如果说过去程序员最重要的问题是“How（怎么实现）”，那么 AI 时代，更重要的问题正在变成“What（实现什么）”和“Why（为什么这样实现）”。代码依然重要，但定义问题、组织上下文和验证结果，正在成为新的核心竞争力。

换句话说，程序员正在从“代码实现者”，逐渐转变为“问题定义者”和“结果验证者”。AI 可以帮助我们完成越来越多的编码工作，但理解业务、拆解问题、设计约束，以及判断一个方案是否真正可用，这些能力依然需要开发者自己完成。总的来说，Vibe Coding 并不是软件工程的终点，也不是程序员价值的终结。它更像是一次开发方式的升级：AI 负责加快实现，人负责定义目标、组织上下文，并通过测试和验证保证最终结果的质量。

未来，优秀程序员的重要能力，或许不再只是写出多少代码，而是提出更好的问题、构建更合理的约束，以及与 AI 高效协作，共同完成一个真正可交付的软件系统。
...
AstrBot 是一款基于 Python 开发的**多平台大语言模型（LLM）聊天机器人及开发框架**，搭配 Vue.js 开发的可视化仪表盘（WebUI），核心目标是降低多平台智能聊天机器人的开发与部署门槛，同时提供灵活的扩展能力。

通俗解释：给聊天工具（QQ）加入智能机器人，然后调用大模型接口，进行各种智能体对话
...
services:
  astrbot:
    ...
    restart: always
    ports:
      - "6185:6185" # WebUI 端口（必选）
      - "6199:6199" # QQ 个人号 WebSocket 端口（可选）
      # 按需开放其他平台端口（如企业微信6195、QQ官方接口6196）
    environment:
      - TZ=Asia/Shanghai
    volumes:
      - ./data:/AstrBot/data # 持久化数据（配置/插件/日志）
...
* 消息平台：填写 QQ/Telegram 等平台的接入信息（如 QQ NapCat 的 WS 地址、Telegram Bot Token）；

* LLM 提供商：配置 API 密钥、模型名称（如 Gemini 的 Thinking Level、OpenAI 的 gpt-4o）；

* 插件管理：安装社区插件（如 `/plugin get 插件仓库地址`）或自定义插件。

​


### openclaw
OpenClaw 龙虾是一款**面向多模态交互、端云协同的开源智能交互引擎**，核心定位是 “轻量化、可扩展的智能硬件交互底座”，其覆盖端侧（Android/iOS/ 嵌入式）、云端、协议层全链路，官网明确了其 “为智能设备提供低延迟、私有化的多模态交互能力” 的核心目标。

通俗解释：传统的 AI 助手只能输出文字类型，OpenClaw 可以直接操作应用（阅读邮件并自动回复），功能更强大，就是非常好用的个人助手。

2026 年现象级 AI 项目，全平台可自托管个人全能 AI 助手，支持 WhatsApp/Telegram/ 微信等所有聊天工具调用，能自主完成代码调试、邮件管理、日历规划、航班值机、文件操作等复杂任务，支持本地 / 云端大模型对接，隐私优先


### build-your-own-x
#### 用途

经典开源学习项目，教你从零实现各类主流技术 / 工具（如数据库、编译器、区块链、AI 模型等），包含详细教程和代码示例，是开发者夯实基础的必备资源
...
免费公共 API 合集，涵盖天气、金融、地图、AI、社交等上万类接口

无需认证 / 低认证门槛，开发快速对接第三方服务的必备工具


### free-programming-books
全网最大的免费编程电子书合集，覆盖上百种语言 / 技术，含英、中、日等多语种，从入门到进阶全阶段资源

### developer-roadmap
开发者职业发展路线图，涵盖前端、后端、AI、DevOps、数据工程师等所有岗位，清晰标注学习路径、技术栈和进阶方向，支持交互式查看

### system-design-primer
系统设计入门与面试必备教程，包含大型分布式系统设计核心原理、经典案例（如抖音、淘宝架构）、Anki 记忆卡片，适配大厂面试

### coding-interview-university
计算机科学完整学习计划，对标大厂算法 / 面试要求，涵盖数据结构、算法、网络、操作系统等核心知识点，从零基础到工程师的全套指南

### prompts.chat
原 Awesome ChatGPT Prompts，全球最大 AI 提示词社区，汇集 ChatGPT/Claude/Gemini 等所有大模型的高质量提示词，支持私有化部署，可自定义企业 / 个人专属提示词库

### system-prompts-and-models-of-ai-tools
聚合 30 + 热门 AI 编程工具（Claude Code/Devin AI/Cursor 等）的原生系统提示词 + 内部模型配置，可直接定制 AI 助手行为，是 AI 开发和调优的核心资源

### Stable Diffusion
经典文本生成图像扩散模型，支持本地部署，可将文字描述转化为高质量图片 / 视频，适配设计、创作、AI 视觉等场景，支持自定义模型和插件

### OpenBB
开源金融数据平台，为分析师、量化交易者、AI 智能体提供股票、期货、外汇等全市场数据，支持可视化分析、API 对接和自动化交易策略开发

### Deer-Flow 2.0
字节跳动开源工业级 AI Agent 调度框架，被称为 “AI 操作系统”，支持子智能体编排、持久化记忆、沙箱隔离，可处理分钟 / 小时级长路径复杂任务（如电商运营、数据分析、自动化测试）

### pi-mono
轻量级 AI Agent 工具包，集成 CLI/TUI/Web UI，提供统一的大模型 API 接入层，支持快速部署自定义智能体，适配中小项目和开发者快速开发

### Agent-Skills-for-Context-Engineering
AI 智能体上下文管理核心技能库，专为多智能体架构、生产级 Agent 系统设计，解决复杂场景下的上下文传递、优化和调试问题

### [文章] 用 Skill 跑通需求+开发全流程
作者：腾讯企业微信团队

[https://zhuanlan.zhihu.com/p/2062565443806697388](https://zhuanlan.zhihu.com/p/2062565443806697388 "https://zhuanlan.zhihu.com/p/2062565443806697388")

#### AI 写业务代码的局限性

| 真实痛点  | 现象                                                 |
| ----- | -------------------------------------------------- |
| 项目代码多 | 源文件几百个、上下文太长、跨 5\~6 层调用，单轮对话喂不进去                   |
| 物料分散  | 需求文档在资料库、设计稿在 Figma、具体调整在企业微信、UI 改动还要看 Figma Token |
| 命名不一致 | 设计师说"描边"，代码里其实叫 border 边框                          |
| 模糊指令  | "改一下xxx"，AI 直接开始改代码，越界漏改改错位                        |
| 验证不闭环 | AI 报"完成"，结果编译没过；改完一处其他地方不正确                        |
| 跨会话失忆 | 上一次的设计决策、改了哪些文件、为什么这么改，下一次会话全忘                     |

一句话总结问题：**AI 不是不会写代码，是不会"按工程规范"开发需求**

#### AI 写业务代码的解决方案：

不是换更大更强的模型，而是把"需求开发"这件事 **流程化、原子化、可校验化**，然后把每一步都喂给 AI。

换成人话：就是按照工作流一步步执行，类似AI开发课程，不能直接从需求变成产品，而是引导 AI 一步步从需求，转换成设计稿，转换成数据库语句，转换成后端 API，转换成页面效果，在每一个过程中确保产出符合预期。如果不符合预期就继续上一步的流程（或者校验需求是否存在问题）。

<img src="https://cloud.seatable.cn/workspace/81910/asset/b0de7002-5abf-48b9-b07b-ba7033be74a7/images/2026-08/image-1785826986499.jpg" alt="" title="" width="466" height="788" />

原作者这里的产品流程比较复杂，实际项目操作可以简化流程
...
| ⑦ 沉淀    | git diff + 时间线         | TECH\_SPEC.md 单一事实源         | 跨会话知识传承的载体                       |
...
这种复杂的 skill，类似于 TS 类型验证。

如果需求明确，团队较小协作很好，比较适合。

如果需求经常变动，那么维护这个 skill 本身就很繁琐，需要一定工作量。

实际开发中，就是把复杂的需求，切成明确的工作流，然后使用 AI 逐步实现，分布进行验证即可。
